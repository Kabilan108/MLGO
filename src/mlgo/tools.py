...
class Enrichment:
    ...
    def dge(
        ...
        if len(groups) > 2:
            raise ValueError(
                f"gse['{group_col}'] should contain only 2 groups"
            )
    ...
    def hgm_tests(self, sig_genes) -> pd.DataFrame:
        ...
        go_tests["p-value"] = go_tests["params"].apply(
            lambda x: stats.hypergeom.sf(*x)
        )
        go_tests["q-value"] = multitest.fdrcorrection(go_tests["p-value"])[1]
        go_tests.drop([0, "params"], axis=1, inplace=True)
        go_tests = go_tests[go_tests["q-value"] < 0.01].reset_index(drop=True)
...
class CuMiDa:
    ...
    def __init__(self, datapath: str = "data/"):
        ...
        data["csv_url"] = (
            CuMiDa.baseurl + data["downloads"].apply(pd.Series)["csv"]
        )

        # Remove datasets with GPLs that don't have GO information
        data = data[
            ~data.Platform.isin(
                ["GPL10553", "GPL17810", "GPL13607", "GPL19197"]
            )
        ]

        self.datasets = (
            data[["ID", "type", "Platform", "classes", "samples", "genes"]]
            .rename(
                {
                    "type": "Type",
                    "classes": "Groups",
        ...
        self.MLPerformance = data.set_index(["ID", "Platform"]).scores.apply(
            pd.Series
        )
    ...
    def download(self, data: pd.DataFrame) -> tuple:
        ...
        self.paths = [
            datapath + re.search(r"\w+\.csv$", url)[0] for url in urls
        ]
    ...
    def load_gpl(self, acc: str):
        ...
        gpl = (
            gpl.table[[idcol, go_col]]
            .set_axis(["ID", "GO"], axis=1)
            .fillna("")
        )
        gpl["GO"] = gpl["GO"].apply(
            lambda x: [f"GO:{y.rjust(7, '0')}" for y in re.findall(pattern, x)]
        )
...
def accuracy_plot(
    go, score, err=None, xlab="", ylab="", title="", filename=None
):
    """
